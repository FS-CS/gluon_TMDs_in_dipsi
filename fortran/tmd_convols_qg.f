c**********
c This version of tmd_convols takes into account the quark component of the gluon PDF. The quark PDFs are evaluated using the MSTW2008 package and then included in the definitions of the different unintegrated convolution functions.
c**********

c     *****************   qMSTW   *****************

      double precision function qMSTWbar(xpdf,bt)
      implicit none
      double precision xpdf,bt
      qMSTWbar = (GetOnePDF(prefix,0,xpdf,mubar(bt),-5)+GetOnePDF(prefix
     &,0,xpdf,mubar(bt),-4)+GetOnePDF(prefix,0,xpdf,mubar(bt),-3)+
     &GetOnePDF(prefix,0,xpdf,mubar(bt),-2)+GetOnePDF(prefix,0,xpdf,
     &mubar(bt),-1)+GetOnePDF(prefix,0,xpdf,mubar(bt),1)+GetOnePDF(
     &prefix,0,xpdf,mubar(bt),2)+GetOnePDF(prefix,0,xpdf,mubar(bt),3)+
     &GetOnePDF(prefix,0,xpdf,mubar(bt),4)+GetOnePDF(prefix,0,xpdf,
     &mubar(bt),5))/xpdf
      end
...
c     y2-integration 
     ...
     &Snp(vbt))*(alphas(vbt)/pi)**2*(1d0/x1-1d0/vy1)*(Nc*gMSTW(
     &vy1,vbt)+Cf*qMSTW(vy1,vbt))*(1d0/x2-1d0/y2)*(Nc*gMSTW(y2,vbt)+
     &Cf*qMSTW(y2,vbt))
      end
...
c     y1-integration 
     ...
     &Snp(vbt))*(alphas(vbt)/pi)*(1d0/x1-1d0/y1)*(Nc*gMSTW(y1,vbt)+
     &Cf*qMSTW(y1,vbt))*gMSTW(x2,vbt)
      end
...
c     y2-integration 
     ...
     &Snp(vbt))*(alphas(vbt)/pi)*(1d0/x2-1d0/y2)*(Nc*gMSTW(y2,vbt)
     &+Cf*qMSTW(y2,vbt))*gMSTW(x1,vbt)
      end
...
c     y2-integration 
     ...
     &Sa(vbt)-Snp(vbt))*(alphas(vbt)/pi)**2*(1d0/x1-1d0/vy1)*
     &(Nc*gMSTW(vy1,vbt)+Cf*qMSTW(vy1,vbt))*(1d0/x2-1d0/y2)*(Nc*
     &gMSTW(y2,vbt)+Cf*qMSTW(y2,vbt))
      end
